...
#define CC_REQUIREMENTS_MSG (ccRequirementsMessage())
...
void SetCCunspents(std::vector<std::pair<CAddressUnspentKey, CAddressUnspentValue> > &unspentOutputs, const char *coinaddr,bool CCflag = true);
...
/// @param coinaddr address where unspent outputs are searched
/// @param CCflag if true the function searches for cc outputs, otherwise for normal outputs
void SetCCunspentsWithMempool(std::vector<std::pair<CAddressUnspentKey, CAddressUnspentValue> > &unspentOutputs, const char *coinaddr, bool ccflag = true);
...
/// SetAddressIndexTxids searches address index for a vector of outputs on an address
...
void SetAddressIndexTxids(std::vector<std::pair<CAddressIndexKey, CAmount>>& addressIndex, const char* coinaddr, bool ccflag, int32_t beginHeight = 0, int32_t endHeight = 0);
