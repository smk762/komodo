...
static void AddCCunspentsInMempool(std::vector<std::pair<CAddressUnspentKey, CAddressUnspentValue> > &unspentOutputs, const char *destaddr, bool isCC)
{
    if (!destaddr) return;
...
void SetCCunspents(std::vector<std::pair<CAddressUnspentKey, CAddressUnspentValue> > &unspentOutputs, const char *coinaddr, bool ccflag)
{
    int32_t type=0,i,n; char *ptr; std::string addrstr; uint160 hashBytes; std::vector<std::pair<uint160, int> > addresses;
    if ( KOMODO_NSPV_SUPERLITE )
    {
        NSPV_CCunspents(unspentOutputs, (char*)coinaddr,ccflag);
        return;
    }
    if (!coinaddr) return;
...
void SetCCunspentsWithMempool(std::vector<std::pair<CAddressUnspentKey, CAddressUnspentValue> > &unspentOutputs, const char *coinaddr, bool ccflag)
{
...
void SetAddressIndexTxids(std::vector<std::pair<CAddressIndexKey, CAmount>>& addressIndex, const char* coinaddr, bool ccflag, int32_t beginHeight, int32_t endHeight)
{
    int32_t type = 0;
    uint160 hashBytes;
    std::vector<std::pair<uint160, int>> addresses;
