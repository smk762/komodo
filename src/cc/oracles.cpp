...
int64_t OracleCurrentDatafee(uint256 reforacletxid,char *markeraddr,CPubKey publisher)
{
    ...
    for (std::vector<std::pair<CAddressUnspentKey, CAddressUnspentValue> >::const_iterator it=unspentOutputs.begin(); it!=unspentOutputs.end(); it++)
    {
        ...
        if ( (GetLatestTimestamp(komodo_currentheight())<MAY2021_NNELECTION_HARDFORK?myGetTransaction(txid,tx,hashBlock)!=0:FetchCCtx(txid,tx,cp)) && (numvouts= tx.vout.size()) > 0 )
        {
