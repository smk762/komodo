...
UniValue OracleRegister(const CPubKey& pk, int64_t txfee, uint256 oracletxid, int64_t datafee)
{
    ...
    if (AddNormalinputs(mtx, mypk, txfee + 2 * CC_MARKER_VALUE, 0x1000, pk.IsValid())) {
        ...
        LOCK(cs_main); // protect komodo_get_current_height()
        if (GetLatestTimestamp(komodo_get_current_height()) > PUBKEY_SPOOFING_FIX_ACTIVATION)  // TODO: we have also komodo_currentheight why not use it?
